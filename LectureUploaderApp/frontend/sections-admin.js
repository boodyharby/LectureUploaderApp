(function(){
  ...
  async function attachSectionsAdmin(){
    ...
    mergeBtn.onclick=async()=>{
      ...
      if(r.ok){ const d=await r.json(); window.toast&&toast('تم الدمج: '+(d.updated||0)); try{ chan&&chan.postMessage({type:'sections_changed', payload:{subject}}); }catch(_){}}
    ...
    delBtn.onclick=async()=>{
      ...
      if(r.ok){ const d=await r.json(); window.toast&&toast('تم الحذف من محاضرات: '+(d.updated||0)); try{ chan&&chan.postMessage({type:'sections_changed', payload:{subject}}); }catch(_){}}
